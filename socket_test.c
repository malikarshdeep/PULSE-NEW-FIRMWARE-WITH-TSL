 ...
 // socket_fd = connect_to_addr("pulseapi.imanifold.com");
  ...
  if (socket_fd < 0) {
   NRF_LOG_INFO("tls_client_not_connected\r\n");
    ...
    NRF_LOG_INFO("check_internet is %d", check_internet );
    vTaskDelay(2000);
    goto repeat;
  }
  NRF_LOG_INFO("tls_client_connected\r\n");
  check_internet = 1;
  if(write_error == 1){
      restart_system =1;
  }
...
//    if ((tickTime - timeout) > send_period) {

//        NRF_LOG_INFO("tick bhai iki valueE %d", tickTime);
//    NRF_LOG_INFO("timeout bhai iki valueE %d", timeout);
//    NRF_LOG_INFO("difference bhai iki valueE %d", tickTime - timeout);
//    NRF_LOG_INFO("send_period bhai iki valueE %d", send_period);
