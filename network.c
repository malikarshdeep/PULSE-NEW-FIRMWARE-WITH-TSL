...
void net_add_enc_netif(void) {
  ...
  {  
   ...
   // sprintf(mac_address, "success mac_%x:%x:%x:%x:%x:%x\n", mac_id[0],mac_id[1],mac_id[2],mac_id[3],mac_id[4],mac_id[5]);

   // NRF_LOG_INFO("DONE MAC");
  }else{
...
//        wifi_conn_info_resp = 0;
  ...
  //  n = snprintf(str, 256, "wifi ap:%d %ddb %s ch:%02d %02X%02X%02X%02X%02X%02X Name:%s\n",
...
//    ret = m2m_wifi_default_connect();
    ...
    {
    //  ip_addr_winc.addr = ipaddr_value;      
    //  if(strncmp(iswifi,"yes",3)==0){
        struct netif * netif_p;
        netif_p = &winc_netif;
        netif_p->ip_addr.addr = ipaddr_value[0];
        netif_p->gw.addr = gw_value[0];
...
void network_task_fn(void *arg) {
  ...
  ret = xTaskCreate(socket_test_task, "socket_test",512*3, NULL,1, &socket_task_handle);
  if (ret != pdPASS)
  {
