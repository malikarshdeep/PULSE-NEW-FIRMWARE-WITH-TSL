...
static const nrfx_spim_t spi = NRFX_SPIM_INSTANCE(PF_SPI_INSTANCE); /**< SPI instance. */
...
void energy_measurement() {
  ...
  spim_xfer.p_tx_buffer = &config_start;
...
if(strcmp(config_string1[23],0x31) == 0 ){
  ...
  //spim_xfer.p_tx_buffer = &vgain_read;
  // spim_xfer.p_rx_buffer = m_rx_buf;
  // APP_ERROR_CHECK(nrfx_spim_xfer(&spi,&spim_xfer, 0));
  //  vTaskDelay(20);
  // memset(m_rx_buf, 0, m_length);
 ...
 // APP_ERROR_CHECK(nrfx_spim_xfer(&spi, &spim_xfer, 0));
 // vTaskDelay(20);
 // float num11 = (m_rx_buf[2] << 8) | (m_rx_buf[3]);
 // sensors_state.activeWatts = num11 * 4 * 3;
 // memset(m_rx_buf, 0, m_length);

 // spim_xfer.p_tx_buffer = &reactive_powerC;
 // spim_xfer.p_rx_buffer = m_rx_buf;
 // APP_ERROR_CHECK(nrfx_spim_xfer(&spi, &spim_xfer, 0));
 // vTaskDelay(20);
 // float num12 = (m_rx_buf[2] << 8) | (m_rx_buf[3]);
 // sensors_state.reactiveWatts = num12 * 4 * 3;
 // memset(m_rx_buf, 0, m_length);
 ...
 // spim_xfer.p_tx_buffer = &igainC_read;
 // spim_xfer.p_rx_buffer = m_rx_buf;
 // APP_ERROR_CHECK(nrfx_spim_xfer(&spi, &spim_xfer, 0));
 // vTaskDelay(20);
 // memset(m_rx_buf, 0, m_length);

  //sensors_state.iUei[2] = ( sensors_state.iUei[0] + sensors_state.iUei[1] )/2;

  // sensors_state.uUei[2] = (sensors_state.activeWatts * 3 * 3)/ (1.73 * sensors_state.powerFactor * (sensors_state.iUei[0] + sensors_state.iUei[1] + sensors_state.iUei[2])) - sensors_state.uUei[0] - sensors_state.uUei[1]   ;
...
thermostat_signals(){
  ...
  //  NRF_LOG_INFO("ORANGE WIRE = %d", sensors_state1.wire1_orange);
  }
...
//NRF_LOG_INFO("WHITE WIRE = %d", sensors_state1.wire2_white);
}
else{  
  vTaskDelay(1);
...
if(nrf_gpio_pin_read(39) == 0){
 ...
 // NRF_LOG_INFO("YELLOW WIRE = %d", sensors_state1.wire3_yellow);
}
else{
vTaskDelay(1);
